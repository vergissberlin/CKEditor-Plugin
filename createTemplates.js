...
var fs        = require('fs'),
	...
	fremeBase = 'http://api.freme-project.eu/0.6/',
	token     = '0c87f536-69bf-4d25-adae-bb9ec9330390';
	...
	function (err, res, templates) {
		...
		fs.readdir(templateBaseDir, function (err, files) {
			async.mapLimit(files, 4, function (file, done) {
				...
				async.map([
					path.resolve(templateBaseDir, file),
					path.resolve(templateBaseDir, file.replace('.txt', '.json'))
				], function (file, readDone) {
